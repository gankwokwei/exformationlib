...
public class Point {
	...
	*/
	...
	public Point div(Point p){
		//returns 0 when division by zero
		//if(isNaN(x))x=0
		//if(isNan(y))y=0
		return new Point(x/=p.x,y/=p.y);
	}
	...
	public void bounce(Rectangle rect, Point vector){
		//offset(vector.x*stepX,vector.y*stepY);
		if(!isXWithinRect(rect)) {vector.x*=-1;}
		if(!isYWithinRect(rect)) {vector.y*=-1;}
		constrain(rect);
	}
	/**
	 ...
	 * */
	static public Point convertToRadialPosition (Point p, Dimension d, int totalElements,float thetaSpan,float startAngle,float innerRadius){

		if(totalElements!=-1) thetaSpan-=(360/totalElements);
		
		float w = d.h;
		float h = d.w;
		float x = (w-p.y)*(1-(1/w*innerRadius))/1;// was: (w-p.y); added the previous to allow inner radius
		float y = p.x;
		float px = (w-x)/2;
		float py = thetaSpan/h*y;
		// make zero degrees be on top, this can cause some headaches later
		py+=startAngle;
		// correct x if aspect ratio is more than 1
		...
		p = new Point(px,py);
		p.toRadial();
		p.sum(h/2,w/2); // this looks backwards but it's correct!
		return p;
	}
	/**
